...
//		echo "<pre>";print_r(EventsHelper::getTyp());echo "</pre>";exit;
		$eventtype = EventsHelper::getTyp();
		// Liste für Typ-Auswahl
		$ret  = '<td>';
			$ret .= '<select name="jform[Typ][]" class="required okt_select">';
			foreach($eventtype as $typ) {
		...
		$ret .= '<td>' .
				'<input name="jform[BezeichnungLink][]" type="text" value="'.@$data->Bezeichnung.'" class="inputbox required" />' .
				'</td>';
		...
		$ret .= '<td>';
			$ret .= '<div class="icon-trash" style="width: 16px; height: 16px; cursor: pointer;" onclick="this.getParent().getParent().destroy();"></div>';
		$ret .= '</td>';
